class GameLayer : Layer {
    ...
    {
        PlayerColor[] playerColors = { PlayerColor.Red, PlayerColor.Green, PlayerColor.Cyan, PlayerColor.Yellow };
        int currentPlayerIndex = 0;
        bool gameWon = false;

        while (!gameWon) // Główna pętla gry
        {
            PlayerColor currentPlayer = playerColors[currentPlayerIndex];

            // kolor konsoli dla aktualnego gracza
            Console.ForegroundColor = currentPlayer switch
            {
            ...
            // ruch
            Console.WriteLine("Rzuć kostką ");

            // Sprawdzenie warunku zwycięstwa
            gameWon = CheckWinCondition(currentPlayer);
            if (gameWon)
            {
                Console.WriteLine($"Gracz {currentPlayer} wygrał grę!");
                break;
            }

            // Resetowanie koloru konsoli
            Console.ResetColor();
...
private bool CheckWinCondition(PlayerColor player)
{
    // Sprawdzenie, czy wszystkie pola w "finish" są true dla danego gracza
    return finish[player].All(field => field);
}
